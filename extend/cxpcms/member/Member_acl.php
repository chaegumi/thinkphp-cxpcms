	...
	class Member_acl
	{
		...
		function __construct($userID = '')
		{
			if ($userID != '')
			{
				...
				$this->userID = floatval(Session::get('member_userid'));
			}
...
/* 		function ACL($userID = '')
		...
		function buildACL()
		{
			if(!$perms = Cache::get('uperms_' . $this->userID)){
				...
				Cache::set('uperms_' . $this->userID, $this->perms, 3600);
			}else{
				$this->perms = $perms;
			}
		} 
		...
		function getUserRoles()
		{
			...
			if($userid){
				if(!$resp = Cache::get('user_roles_' . $userid)){
					...
					Cache::set('user_roles_' . $userid, $resp, 3600);
				}
				return $resp;
			}else{
				return array();
		...
		{
			if($userID){
				if(!$perms = Cache::get('userperms_' . $userID)){
					...
					Cache::set('userperms_' . $userID, $perms, 3600);
				}
				return $perms;
			}else{
				return array();
		...
		function hasPermission($permKey)
		{
			...
			{
				if (intval($this->perms[$permKey]['value']) === 1 || $this->perms[$permKey]['value'] === true)
				{
					return true;
				} else {
					return false;
		...
		function getPermArr($type = ''){
			if($type == ''){
				...
				foreach($perms as $k=>$perm){
					$newperm[$k] = (intval($perm['value']) === 1 || $perm['value'] === true ? true : false);
				}
				return $newperm;
			}
